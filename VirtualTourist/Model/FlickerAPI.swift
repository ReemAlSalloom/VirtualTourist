...
struct FlickrData
{
   ...
   // let path = "/services/rest"
    ...
    func obtainData(longitude: Double, latitude: Double, page: Int32, completion: @escaping (_ error: String?, _ data: [String]?) -> ())
    {
        ...
        let task = session.dataTask(with: URL(string: baseURL)!) { data, responce, error in
            ...
            if let photos = jsonData["photo"] as? [String: Any], let photo = photos["photo"] as? [[String: Any]]
            {
                var count = photos.count - 1
                ...
                while count > 0
                {
                    ...
                    let objCount = photo[count]
                    let photoUrl = objCount["url_m"] as! String
